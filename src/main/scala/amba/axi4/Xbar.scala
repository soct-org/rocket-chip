...
class AXI4Xbar(
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val awIn = Seq.fill(io_in.size) {
      Module(new Queue(UInt(io_out.size.W), awQueueDepth, flow = true))
    }
    val awOut = Seq.fill(io_out.size) {
      Module(new Queue(UInt(io_in.size.W), awQueueDepth, flow = true))
    }
    ...
    for (i <- 0 until io_in.size) {
      awIn(i).io.enq.bits := requestAWIO(i).asUInt
    }
    ...
    for (i <- 0 until in.size) {
      ...
      if (io_out.size > 1) {
        ...
        val arFIFOMap = WireDefault(VecInit.fill(endId) {
          true.B
        })
        val awFIFOMap = WireDefault(VecInit.fill(endId) {
          true.B
        })
...
object AXI4Arbiter {
  ...
  def returnWinner[T <: Data](policy: TLArbiter.Policy)(sink: IrrevocableIO[T], sources: IrrevocableIO[T]*) = {
    ...
    assert((prefixOR zip winner) map { case (p, w) => !p || !w } reduce {
      _ && _
    })
...
class AXI4XbarFuzzTest(name: String, txns: Int, nMasters: Int, nSlaves: Int)(implicit p: Parameters) extends LazyModule {
  ...
  val masters = Seq.fill(nMasters) {
    LazyModule(new TLFuzzer(txns, 4, nOrdered = Some(1)))
  }
