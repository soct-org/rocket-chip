...
class AXI4ToTL(wcorrupt: Boolean)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val r_count = RegInit(VecInit.fill(numIds) {
        0.U(txnCountBits.W)
      })
      ...
      val w_count = RegInit(VecInit.fill(numIds) {
        0.U(txnCountBits.W)
      })
      ...
      in.w.bits.user.lift(AMBACorrupt).foreach {
        w_out.bits.corrupt := _
      }
      ...
      val b_count = RegInit(VecInit.fill(numIds) {
        0.U(txnCountBits.W)
      })
