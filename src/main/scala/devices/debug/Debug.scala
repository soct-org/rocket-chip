...
object DsbRegAddrs {
  ...
  def IMPEBREAK(cfg: DebugModuleParams) = {
    DATA - 4
  }
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val omRegMap = withReset(reset.asAsyncReset) {
      ...
      when(~dmactive) {
        ...
        when(dmAuthenticated && ndmresetWrEn) {
          DMCONTROLNxt.ndmreset := DMCONTROLWrData.ndmreset
        }
        when(dmAuthenticated && hartselloWrEn) {
          DMCONTROLNxt.hartsello := DMCONTROLWrData.hartsello & hartsel_mask
        }
        when(dmAuthenticated && haselWrEn) {
          DMCONTROLNxt.hasel := DMCONTROLWrData.hasel
        }
        when(dmAuthenticated && hartresetWrEn) {
          DMCONTROLNxt.hartreset := DMCONTROLWrData.hartreset
        }
        when(dmAuthenticated && haltreqWrEn) {
      ...
      val debugIntNxt = WireInit(VecInit(Seq.fill(nComponents) {
        false.B
      }))
...
class TLDebugModuleInner(device: Device, getNComponents: () => Int, beatBytes: Int)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val dmiProgramBufferRdEn = WireInit(VecInit(Seq.fill(cfg.nProgramBufferWords * 4) {
      false.B
    }))
    val dmiProgramBufferAccessLegal = WireInit(false.B)
    val dmiProgramBufferWrEnMaybe = WireInit(VecInit(Seq.fill(cfg.nProgramBufferWords * 4) {
      false.B
    }))

    val dmiAbstractDataRdEn = WireInit(VecInit(Seq.fill(cfg.nAbstractDataWords * 4) {
      false.B
    }))
    val dmiAbstractDataAccessLegal = WireInit(false.B)
    val dmiAbstractDataWrEnMaybe = WireInit(VecInit(Seq.fill(cfg.nAbstractDataWords * 4) {
      false.B
    }))
    ...
    val hamaskFull = WireInit(VecInit(Seq.fill(nComponents) {
      false.B
    }))
    ...
    if (supportHartArray) {
      val hamaskZero = WireInit(VecInit(Seq.fill(nComponents) {
        false.B
      }))
    ...
    val hamaskWrSel = WireInit(VecInit(Seq.fill(nComponents) {
      false.B
    }))
    for (component <- 0 until nComponents) {
    ...
    //-------------------------------------
    val hrReset = WireInit(VecInit(Seq.fill(nComponents) {
      false.B
    }))
    ...
    withReset(reset.asAsyncReset) { // ensure interrupt requests are negated at first clock edge
      val hrDebugIntReg = RegInit(VecInit(Seq.fill(nComponents) {
        false.B
      }))
      when(~io.dmactive || ~dmAuthenticated) {
    ...
    val hgDebugInt = WireInit(VecInit(Seq.fill(nComponents) {
      false.B
    }))
    ...
    if (nHaltGroups > 0) withReset(reset.asAsyncReset) { // async reset ensures triggers don't falsely fire during startup
      ...
      val hgFired = RegInit(VecInit(Seq.fill(nHaltGroups + 1) {
        false.B
      }))
      val hgHartFiring = WireInit(VecInit(Seq.fill(nHaltGroups + 1) {
        false.B
      })) // which hg's are firing due to hart halting
      val hgTrigFiring = WireInit(VecInit(Seq.fill(nHaltGroups + 1) {
        false.B
      })) // which hg's are firing due to trig in
      ...
      io.extTrigger.foreach { extTrigger =>
        val extTriggerOutReq = RegInit(VecInit(Seq.fill(cfg.nExtTriggers) {
          false.B
        }))
    ...
    val dmiAbstractDataAccessVec = WireInit(VecInit(Seq.fill(cfg.nAbstractDataWords * 4) {
      false.B
    }))
    ...
    val dmiProgramBufferAccessVec = WireInit(VecInit(Seq.fill(cfg.nProgramBufferWords * 4) {
      false.B
    }))
    ...
    val autoexecData = WireInit(VecInit(Seq.fill(cfg.nAbstractDataWords) {
      false.B
    }))
    val autoexecProg = WireInit(VecInit(Seq.fill(cfg.nProgramBufferWords) {
      false.B
    }))
    ...
    val flags = WireInit(VecInit(Seq.fill(1 << selectedHartReg.getWidth) {
      0.U.asTypeOf(new flagBundle())
    }))
    ...
    tlNode.regmap(
      ...
      IMPEBREAK(cfg) -> {
        if (cfg.hasImplicitEbreak) Seq(RegField.r(32, Instructions.EBREAK.value.U,
