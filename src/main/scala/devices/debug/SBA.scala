...
object SystemBusAccessModule {
  ...
  (Seq[RegField], Seq[Seq[RegField]], Seq[Seq[RegField]]) = {
    ...
    val SBADDRESSWrData = WireInit(VecInit(Seq.fill(4) {
      0.U(32.W)
    }))
    val SBADDRESSRdEn = WireInit(VecInit(Seq.fill(4) {
      false.B
    }))
    val SBADDRESSWrEn = WireInit(VecInit(Seq.fill(4) {
      false.B
    }))
    ...
    val SBDATARdData = WireInit(VecInit(Seq.fill(4) {
      0.U(32.W)
    }))
    SBDATARdData.zipWithIndex.foreach { case (d, i) => d.suggestName("SBDATARdData" + i) }
    val SBDATAWrData = WireInit(VecInit(Seq.fill(4) {
      0.U(32.W)
    }))
    SBDATAWrData.zipWithIndex.foreach { case (d, i) => d.suggestName("SBDATAWrData" + i) }
    val SBDATARdEn = WireInit(VecInit(Seq.fill(4) {
      false.B
    }))
    val SBDATAWrEn = WireInit(VecInit(Seq.fill(4) {
      false.B
    }))
...
class SBToTL(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    when(sbState === SBReadRequest.id.U) {
      tl.a.bits := gbits
    }
      .otherwise {
        tl.a.bits := pfbits
