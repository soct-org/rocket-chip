...
case class HierarchicalElementMasterPortParams(
  ...
  def injectNode(context: Attachable)(implicit p: Parameters): TLNode = {
    (TLBuffer.chainNode(buffers) :=* cork.map { u => TLCacheCork(unsafe = u) }.getOrElse {
      TLTempNode()
    })
  }
}
...
case class HierarchicalElementSlavePortParams(
  ...
  def injectNode(context: Attachable)(implicit p: Parameters): TLNode = {
    ...
    blockerCtrlAddr
      .map {
        BasicBusBlockerParams(_, blockerBus.beatBytes, controlBus.beatBytes)
      }
      .map { bbbp =>
        val blocker = LazyModule(new BasicBusBlocker(bbbp))
        blockerBus.coupleTo("tile_slave_port_bus_blocker") {
          blocker.controlNode := TLFragmenter(blockerBus) := _
        }
        blocker.node :*= TLBuffer.chainNode(buffers)
      }.getOrElse {
        TLBuffer.chainNode(buffers)
      }
  }
}
 ...
 */
trait InstantiatesHierarchicalElements {
...
trait HasHierarchicalElementsRootContext {
  ...
  for (i <- 0 until nTotalTiles) {
    meipNodes.get(i).foreach {
      _ := plicOpt.map(_.intnode).getOrElse(meipIONode.get)
    }
    seipNodes.get(i).foreach {
      _ := plicOpt.map(_.intnode).getOrElse(seipIONode.get)
    }
  }
