...
trait HasConfigurablePRCILocations {
  ...
  private lazy val busHierarchyByClock: Map[ClockBundle, Set[BusWithFreq]] =
    ...
    busHierMap.map { case (clk, busSet) => clk -> busSet.toSet }.toMap
  }
  ...
  /**
   * Returns the set of [[TLBusWrapper]]s driven by the given [[ClockBundle]], if any exist.
   *
   * This can be used to find all TL buses in the subsystem topology that are driven by a particular clock bundle,
