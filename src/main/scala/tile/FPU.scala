    ...
    when(!in.ren2) { // fcvt
      ...
      for (i <- 0 until nIntTypes - 1) {
        ...
        when(cvtType === i.U) {
          ...
          when(invalid) {
            toint := Cat(conv.io.out >> w, excOut)
          }
...
class FPU(cfg: FPUParams)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  coreParams match {
    case r: RocketCoreParams => r.vector.map(v => {
  ...
  class FPUImpl { // entering gated-clock domain
    ...
    if (cfg.divSqrt) {
      ...
      for (t <- floatTypes) {
        ...
        val divSqrt = withReset(divSqrt_killed) {
          Module(new hardfloat.DivSqrtRecFN_small(t.exp, t.sig, 0))
        }
        ...
        when(!divSqrt.io.inReady) {
          divSqrt_inFlight := true.B
        } // only 1 in flight
      ...
      when(divSqrt_killed) {
        divSqrt_inFlight := false.B
      }
    } else {
      when(id_ctrl.div || id_ctrl.sqrt) {
        io.illegal_rm := true.B
      }
    }
