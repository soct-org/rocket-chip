    ...
    case sp =>
      val managers = sp.slaves.map { case s =>
        TLSlaveParameters.v1(
          ...
          supportsHint = if (!supportHints) TransferSizes.none else if (s.supportsRead) s.supportsRead else if (s.supportsWrite) s.supportsWrite else
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      when(out.hready && granted && !retry) {
        ...
        when(out.busreq() && !out.grant()) {
          increment := true.B
        }
        when(send.send && send.last) {
          increment := false.B
        }
      }
