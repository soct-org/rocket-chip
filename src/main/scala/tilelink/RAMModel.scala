...
class TLRAMModel(log: String = "", ignoreCorruptData: Boolean = false, ignoreDeniedData: Boolean = true)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    (node.in zip node.out) foreach { case ((in, edgeIn), (out, edgeOut)) =>
      ...
      val shadow = Seq.fill(beatBytes) {
        Mem(endAddressHi, new TLRAMModel.ByteMonitor(params))
      }
      val inc_bytes = Seq.fill(beatBytes) {
        Mem(endAddressHi, UInt(countBits.W))
      }
      val dec_bytes = Seq.fill(beatBytes) {
        Mem(endAddressHi, UInt(countBits.W))
      }
      ...
      when(a_fire) {
        ...
        when(a.opcode === TLMessages.PutFullData || a.opcode === TLMessages.PutPartialData ||
          ...
          for (i <- 0 until beatBytes) {
            ...
            when(a.mask(i)) {
              printf(log + " ")
              when(a.opcode === TLMessages.PutFullData) {
                printf("PF")
              }
              when(a.opcode === TLMessages.PutPartialData) {
                printf("PP")
              }
              when(a.opcode === TLMessages.ArithmeticData) {
                printf("A ")
              }
              when(a.opcode === TLMessages.LogicalData) {
                printf("L ")
              }
      ...
      when(d_fire) {
        ...
        when(d_flight.opcode === TLMessages.PutFullData || d_flight.opcode === TLMessages.PutPartialData) {
          ...
          printf(log + " ")
          when(d_flight.opcode === TLMessages.PutFullData) {
            printf("pf")
          }
          when(d_flight.opcode === TLMessages.PutPartialData) {
            printf("pp")
          }
        ...
        when(d_flight.opcode === TLMessages.Get || d_flight.opcode === TLMessages.ArithmeticData || d_flight.opcode === TLMessages.LogicalData) {
          ...
          for (i <- 0 until beatBytes) {
            ...
            when(d_mask(i)) {
              ...
              when(d_flight.opcode === TLMessages.ArithmeticData) {
                printf("a ")
              }
              when(d_flight.opcode === TLMessages.LogicalData) {
                printf("l ")
              }
        ...
        when(d_flight.opcode === TLMessages.ArithmeticData || d_flight.opcode === TLMessages.LogicalData) {
          ...
          when((Cat(race.reverse) & d_mask).orR) {
            d_no_race := false.B
          }
          when(d_last) {
            ...
            when(!corrupt && must_match && d_crc =/= d_crc_check) {
              printf("EXPECTED: 0x%x\n", d_crc_check)
            }
