...
abstract class TLBusWrapper(params: HasTLBusParams, val busName: String)(implicit p: Parameters)
  ...
  }).suggestName(busName + "_clock_groups")
  ...
  def to[T](name: String)(body: => T): T = {
    this {
      LazyScope(s"coupler_to_${name}", s"TLInterconnectCoupler_${busName}_to_${name}") {
        body
      }
    }
  }
  ...
  def from[T](name: String)(body: => T): T = {
    this {
      LazyScope(s"coupler_from_${name}", s"TLInterconnectCoupler_${busName}_from_${name}") {
        body
      }
    }
  }
  ...
  def coupleTo[T](name: String)(gen: TLOutwardNode => T): T =
    to(name) {
      gen(TLNameNode("tl") :*=* outwardNode)
    }

  def coupleFrom[T](name: String)(gen: TLInwardNode => T): T =
    from(name) {
      gen(inwardNode :*=* TLNameNode("tl"))
    }
...
class TLBusWrapperTopology(
  ...
  def instantiate(context: HasTileLinkLocations)(implicit p: Parameters): Unit = {
    instantiations.foreach { case (loc, params) => context {
      params.instantiate(context, loc)
    }
    }
  }

  def connect(context: HasTileLinkLocations)(implicit p: Parameters): Unit = {
    connections.foreach { case (master, slave, params) => context {
      params.connect(context, master, slave)
    }
    }
  }
}
