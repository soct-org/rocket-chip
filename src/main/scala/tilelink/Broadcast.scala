...
class TLBroadcast(params: TLBroadcastParams)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    val (ints, fields) = node.in.zip(node.out).zipWithIndex.map { case (((in, edgeIn), (out, edgeOut)), bankIndex) =>
      ...
      val c_trackerSrc = Mux1H(c_trackerOH, trackers.map {
        _.source
      })
      ...
      when(in.b.fire) {
        probe_todo := probe_todo & ~probe_next
      }
    ...
    intNode.foreach {
      _.out(0)._1(0) := ints.reduce(_ || _)
    }
    controlNode.foreach {
      _.regmap(fields.flatten: _*)
    }
  }
}
