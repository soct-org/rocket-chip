...
class IBuf(implicit p: Parameters) extends CoreModule {
  ...
  def expand(i: Int, j: UInt, curInst: UInt): Unit = if (i < retireWidth) {
    ...
    if (usingCompressed) {
      ...
      when((bufMask(j) && exp.io.rvc) || bufMask(j + 1.U)) {
        io.btb_resp := ibufBTBResp
      }

      when(full_insn && ((i == 0).B || io.inst(i).ready)) {
        nReady := Mux(exp.io.rvc, j + 1.U, j + 2.U)
      }

      expand(i + 1, Mux(exp.io.rvc, j + 1.U, j + 2.U), Mux(exp.io.rvc, curInst >> 16, curInst >> 32))
    } else {
      when((i == 0).B || io.inst(i).ready) {
        nReady := (i + 1).U
      }
