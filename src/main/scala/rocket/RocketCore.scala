...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  class RocketImpl { // entering gated-clock domain
    ...
    when(!ctrl_killd) {
      ...
      ex_ctrl.csr := id_csr
      when(id_ctrl.fence && id_fence_succ === 0.U) {
        id_reg_pause := true.B
      }
      when(id_fence_next) {
        id_reg_fence := true.B
    ...
    if (rocketParams.enableTraceCoreIngress) {
      ...
      io.trace_core_ingress.get.ctx := csr.io.ptbr.asidio.trace_core_ingress.get.tval := csr.io.tval
  ...
  val rocketImpl = withClock(gated_clock) {
    new RocketImpl
  }
