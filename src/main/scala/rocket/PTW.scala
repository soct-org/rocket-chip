    ...
    val (pte, invalid_paddr, invalid_gpa) = {
      ...
      when(tmp.r || tmp.w || tmp.x) {
        ...
        for (i <- 0 until pgLevels - 1)
          when(count <= i.U && tmp.ppn((pgLevels - 1 - i) * pgLevelBits - 1, (pgLevels - 2 - i) * pgLevelBits) =/= 0.U) {
            res.v := false.B
          }
      }
      (res,
...
trait CanHavePTW extends HasTileParameters with HasHellaCache {
  this: BaseTile =>
