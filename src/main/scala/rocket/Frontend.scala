...
class FrontendModule(outer: Frontend) extends LazyModuleImp(outer)
  ...
  val fq = withReset(reset.asBool || io.cpu.req.valid) {
    Module(new ShiftQueue(new FrontendResp, 5, flow = true))
  }
  ...
  withClock(gated_clock) { // entering gated-clock domain
    ...
    when(io.ptw.req.fire && recent_progress) {
      recent_progress_counter := recent_progress_counter - 1.U
    }
    when(io.cpu.progress) {
      recent_progress_counter := recent_progress_counter_init
    }
    ...
    when(icache.io.resp.valid && icache.io.resp.bits.ae) {
      fq.io.enq.bits.xcpt.ae.inst := true.B
    }
    ...
    if (usingBTB) {
      ...
      when(io.ptw.customCSRs.flushBTB) {
        btb.io.flush := true.B
      }
      when(force_taken) {
...
trait HasICacheFrontend extends CanHavePTW {
  ...
  frontend.icache.hartIdSinkNodeOpt.foreach {
    _ := hartIdNexusNode
  }
  frontend.icache.mmioAddressPrefixSinkNodeOpt.foreach {
    _ := mmioAddressPrefixNexusNode
  }
