...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  class DCacheModuleImpl { // entering gated-clock domain
    ...
    when(!s1_tlb_req_valid && s1_valid && s1_cmd_uses_tlb && tlb.io.resp.miss) {
      s1_nack := true.B
    }
    ...
    when(s1_tlb_req_valid && s1_valid && !(s1_req.phys && s1_req.no_xcpt)) {
      s1_nack := true.B
    }
    ...
    val s2_no_alloc_hazard = if (!usingVM || pgIdxBits >= untagBits) false.B else {
      ...
      when(!uncachedInFlight.asUInt.orR) {
        any_no_alloc_in_flight := false.B
      }
      when(s2_valid && s2_req.no_alloc) {
        any_no_alloc_in_flight := true.B
      }
    ...
    when(io.cpu.s2_nack || (s2_valid_hit_pre_data_ecc_and_waw && s2_update_meta)) {
      s1_nack := true.B
    }
    ...
    when(s2_valid_not_killed && lrscValid) {
    ...
    when(blockProbeAfterGrantCount > 0.U) {
      blockProbeAfterGrantCount := blockProbeAfterGrantCount - 1.U
    }
    ...
    if (supports_flush) {
      ...
      when(tl_out_a.fire && !s2_uncached) {
        flushed := false.B
      }
      when(flushing) {
    ...
    io.cpu.perf.blocked := {
      ...
      val near_end_of_refill = if (cacheBlockBytes / beatBytes <= beatsBeforeEnd) tl_out.d.valid else {
        ...
        when(tl_out.d.fire && grantIsRefill) {
  ...
  val dcacheImpl = withClock(gated_clock) {
    new DCacheModuleImpl
  }
