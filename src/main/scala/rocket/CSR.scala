...
class CSRFile(
  ...
  val reg_wfi = withClock(io.ungated_clock) {
    RegInit(false.B)
  }
  ...
  else withClock(io.ungated_clock) {
    WideCounter(64, !io.csr_stall, inhibit = reg_mcountinhibit(0))
  }
  ...
  io.interrupts.seip.foreach {
    mip.seip := reg_mip.seip || _
  }
  ...
  when(insn_wfi && !io.singleStep && !reg_debug) {
    reg_wfi := true.B
  }
  when(pending_interrupts.orR || io.interrupts.debug || exception) {
    reg_wfi := false.B
  }
  io.interrupts.nmi.map(nmi => when(nmi.rnmi) {
    reg_wfi := false.B
  })

  when(io.retire(0) || exception) {
    reg_singleStepped := true.B
  }
  when(!io.singleStep) {
    reg_singleStepped := false.B
  }
  ...
  when(csr_wen) {
    val scause_mask = ((BigInt(1) << (xLen - 1)) + 31).U
    ...
    if (usingNMI) {
      ...
      when(decoded_addr(CustomCSRs.mnscratch)) {
        reg_mnscratch := wdata
      }
      when(decoded_addr(CustomCSRs.mnepc)) {
        reg_mnepc := formEPC(wdata)
      }
      when(decoded_addr(CustomCSRs.mncause)) {
        reg_mncause := wdata & ((BigInt(1) << (xLen - 1)) + BigInt(3)).U
      }
    ...
    if (usingDebug) {
      ...
      when(decoded_addr(CSRs.dscratch0)) {
        reg_dscratch0 := wdata
      }
    ...
    if (usingSupervisor) {
      ...
      when(decoded_addr(CSRs.senvcfg)) {
        reg_senvcfg.write(wdata)
      }
    }
    ...
    if (usingHypervisor) {
      ...
      when(decoded_addr(CSRs.hideleg)) {
        reg_hideleg := wdata
      }
      when(decoded_addr(CSRs.hedeleg)) {
        reg_hedeleg := wdata
      }
      ...
      when(decoded_addr(CSRs.hcounteren)) {
        reg_hcounteren := wdata
      }
      when(decoded_addr(CSRs.htval)) {
        reg_htval := wdata
      }
      when(decoded_addr(CSRs.mtval2)) {
        reg_mtval2 := wdata
      }

      val write_mhtinst_read_pseudo = wdata(13) && (xLen == 32).option(true.B).getOrElse(wdata(12))
      when(decoded_addr(CSRs.mtinst)) {
        reg_mtinst_read_pseudo := write_mhtinst_read_pseudo
      }
      when(decoded_addr(CSRs.htinst)) {
        reg_htinst_read_pseudo := write_mhtinst_read_pseudo
      }
      ...
      when(decoded_addr(CSRs.vsie)) {
        reg_mie := (reg_mie & ~read_hideleg) | ((wdata << 1) & read_hideleg)
      }
      when(decoded_addr(CSRs.vsscratch)) {
        reg_vsscratch := wdata
      }
      when(decoded_addr(CSRs.vsepc)) {
        reg_vsepc := formEPC(wdata)
      }
      when(decoded_addr(CSRs.vstvec)) {
        reg_vstvec := wdata
      }
      when(decoded_addr(CSRs.vscause)) {
        reg_vscause := wdata & scause_mask
      }
      when(decoded_addr(CSRs.vstval)) {
        reg_vstval := wdata
      }
      when(decoded_addr(CSRs.henvcfg)) {
        reg_henvcfg.write(wdata)
      }
    }
    if (usingUser) {
  ...
  def writeCounter(lo: Int, ctr: WideCounter, wdata: UInt) = {
    if (xLen == 32) {
      ...
      when(decoded_addr(lo)) {
        ctr := Cat(ctr(ctr.getWidth - 1, 32), wdata)
      }
      when(decoded_addr(hi)) {
        ctr := Cat(wdata(ctr.getWidth - 33, 0), ctr(31, 0))
      }
    } else {
      when(decoded_addr(lo)) {
        ctr := wdata(ctr.getWidth - 1, 0)
      }
    }
  }
