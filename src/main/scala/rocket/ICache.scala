...
class ICacheModule(outer: ICache) extends LazyModuleImp(outer)
  ...
  when(refill_done) {
    ...
    tag_array.write(refill_idx, VecInit(Seq.fill(nWays) {
      enc_tag
  ...
  for ((data_array, i) <- data_arrays.zipWithIndex) {
    ...
    when(wen) {
      ...
      data_array.write(mem_idx, VecInit(Seq.fill(nWays) {
        dECC.encode(data)
      }), (0 until nWays).map(way === _.U))
    }
    // write access
    ...
    case 2 =>
      ...
      tl_in.map { tl =>
        ...
        when(s2_slaveValid) {
          ...
          when(edge_in.get.hasData(s1_a) || s2_data_decoded.error) {
            s3_slaveValid := true.B
          }
